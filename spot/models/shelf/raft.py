...
class RAFT(nn.Module):
    def __init__(self, args, mixed_precision=False):
        super().__init__()
    ...
    def forward(self, src_frame=None, tgt_frame=None, src_feats=None, tgt_feats=None, coarse_flow=None, coarse_alpha=None,
                is_train=False, num_iter=None, alpha_thresh=0.8):
        src_frame = src_frame * 2 - 1
        tgt_frame = tgt_frame * 2 - 1
        src_frame = src_frame.contiguous()
        tgt_frame = tgt_frame.contiguous()

        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            src_fmap, tgt_fmap = self.fnet([src_frame, tgt_frame])
            src_cmap = self.cnet(src_frame)

        src_fmap, tgt_fmap = src_fmap.float(), tgt_fmap.float()
        ...
        return {"flow": flows_up, "alpha": alphas_up}
